...
public class MariaDBSession extends RemoteSQLSession {
    ...
    public MariaDBSession(AgoniaGuilds plugin, boolean logging) {
        super(plugin, logging);
    }

    @Override
    public boolean createConnection() {
        if (logging) Log.info("Försöker att ansluta till avlägsen databas (MariaDB)...");
        ...
        try {
            ...
            Log.error(error, "Ett oförväntat fel uppstod vid anslutning till MariaDB databasen:");
        }
    ...
    public void setJournalMode(String jounralMode, QueryResult<ResultSet> queryResult) {
        queryResult.fail(new UnsupportedOperationException("Omöjligt att ändra Journal läge i MariaDB."));
    }
